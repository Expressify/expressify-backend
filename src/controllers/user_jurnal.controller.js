import {
  ...
  updateOne,
  deleteOne,
} from "../services/user_jurnal.service.js";

const getAllUserJurnalController = async (req, res) => {
  try {
    return res.status(200).json({
      data: await getAll(),
      message: "success fetching user_jurnal",
      status: true,
    });
  } catch (err) {
...
const getOneUserJurnalController = async (req, res) => {
  try {
    return res.status(200).json({
      data: await getOne(req.params.id),
      message: "Success fetching user_jurnal data",
      status: true,
    });
  } catch (err) {
    return res.status(500).json({
...
const createOneUserJurnalController = async (req, res) => {
  try {
...
const updateOneUserJurnalController = async (req, res) => {
  try {
